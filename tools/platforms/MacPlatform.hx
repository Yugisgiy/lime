...
class MacPlatform extends PlatformTarget
{
	...
	public function new(command:String, _project:HXProject, targetFlags:Map<String, String>)
	{
		...
		{
			if (architecture.match(X86 | X64 | ARMV7 | ARM64))
			{
	...
	public override function build():Void
	{
		...
		else if (targetType == "hl")
		{
			...
			if (project.targetFlags.exists("hlc"))
			{
				...
				var command = ["arch", "-x86_64", compiler, "-O3", "-o", executablePath, "-Wl,-rpath,@executable_path", "-I", Path.combine(targetDirectory, "obj"), Path.combine(targetDirectory, "obj/ApplicationMain.c")];
